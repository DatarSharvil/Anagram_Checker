# About:
  A CLI tool that checks Anagrams in CLI on Linux x64 (Debian based and non-Debian based) and Windows 8+ x64 and macOS (macOS 11+) without an internet. Enjoy!
# Requirements:
  <p> Minimum: Java 21 </p>
  <p> Recommended: Java 21 </p>

# Usage:
  1. Linux (Debian-based): Just install the .deb file the way you like and enter the command "anagram" from any terminal to execute.
  2. Linux (Non-Debian-based): Extract the tar.gz file and run the script using a terminal.
  3. Windows: Double click the .exe to install the application and launch it from the start menu to launch it, or add it to PATH to launch it from CMD.
  4. macOS: Same as Non-Debian based Linux.
<p>
Note: If problems like missing JRE with .exe, make sure JDK 17+ is installed. If still fails extract the .tar.gz file from releases and run the .jar file by running this in CMD: "java -jar anagram.jar" in the same directory. (Not applicable for v1.3+ anymore.)
